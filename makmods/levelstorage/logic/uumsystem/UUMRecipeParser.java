				...
				try {
					...
					for (Object i : inputs) {
						if (i instanceof ItemStack) {
							if (Helper.compareStacksGenerally((ItemStack)i, UUM)) {
								uumInRecipe++;
							} else {
								onlyUUMRecipe = false;
								break;
					...
					if (onlyUUMRecipe && (uumInRecipe > 0)) {
						...
						FMLLog.log(Level.INFO, Reference.MOD_NAME + ": UUM Recipe - ");
						FMLLog.log(Level.INFO, "\t UUM Cost: " + doneRec.getUumCost() + " UUM");
						FMLLog.log(Level.INFO, "\t Output: " + Helper.getNiceStackName(doneRec.getOutput()));
						this.recipes.add(doneRec);
					}
					
				} catch (Exception e) {
					FMLLog.severe(Reference.MOD_NAME + ": something went wrong when parsing UUM recipes");
					e.printStackTrace();
				}
			}
		}
		FMLLog.log(Level.INFO, Reference.MOD_NAME + ": finished parsing UUM recipes.");
	}
